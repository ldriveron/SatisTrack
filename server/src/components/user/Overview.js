...
const Overview = (props) => {
	...
	useEffect(
		() => {
			...
			api.fetchSingularUser(props.match.params.username).then(async (user) => {
				setUser(user.user);

				if (user.user.error == 0) {
					if (user.user.private == false) {
						await api.fetchPublicOverview(user.user.id).then((resp) => {
	...
	if (user.private == false && user.error == 0) {
		return (
			<div className="dashboard">
				<div className="page_title">{user.username}&apos;s Public Overview</div>
			...
			<div className="dashboard">
				...
				<div className="monthly_report">
					{isLoading ? (
						...
						<div className="overview_report" style={{ marginTop: '30px' }}>
							<div className="user_alert">
								{props.match.params.username}&apos;s Overview is set to private or no user was found.
							</div>
						</div>
					)}
				</div>
